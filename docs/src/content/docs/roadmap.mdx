...
Still planned: `/rules` command to list loaded rules with scope and path
filters, and `/rules:add` to create new rule files.
  ...
  <div class="roadmap-meta">
    ...
    <span class="roadmap-meta-item"><strong>Shipped:</strong> Unconditional + path-scoped rule loading (`paths`/`path`) across .tallow and .claude rules directories</span>
  </div>
</div>
