...
first. Select a turn to roll back to, confirm, and tracked/unignored
files are restored to that point. Files created after the selected turn
are deleted when they are tracked or unignored.
...
2. **Git snapshots** — At each turn boundary (`turn_end`), creates a
   lightweight git ref from git's tracked + unignored view of the
   working tree. That captures normal edit/write changes and unignored
   bash side effects without touching the user's real index.
3. **Restore** — Uses `git checkout <ref> -- .` to atomically restore
   tracked snapshot files, then deletes files that were created after
   the snapshot when git reports them as tracked or unignored. The
   user's original git index is restored afterward so existing staged
   work survives rewind.
...
- **Ignored files are out of scope** — `.gitignore`d files are not part
  of the snapshot or restore set.
