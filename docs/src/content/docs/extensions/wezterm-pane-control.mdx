...
> **Looking for the full picture?** The [WezTerm Integration
> guide](/guides/wezterm-integration/) covers this extension plus
> turn status signaling, tab bar indicators, and WezTerm Lua
> configuration.
...
The guidance also tells the model to avoid opening new panes/tabs
unless the user explicitly asks for pane/tab management, and to
prefer `bg_bash` in the current session for long-running commands
like dev servers and watchers unless the user asks otherwise.
