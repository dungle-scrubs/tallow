...
- agent hooks resolve their runner in this order:
  `TALLOW_HOOK_AGENT_RUNNER` override → current tallow executable
  (when detectable) → `tallow` on PATH → legacy `pi` fallback
...
5. `.tallow/settings.json` → `hooks` key (project settings, **trusted projects only**)
...
8. `.claude/settings.json` → `hooks` key (project Claude hooks, translated)
9. `~/.claude/settings.json` → `hooks` key (global Claude hooks, translated)
