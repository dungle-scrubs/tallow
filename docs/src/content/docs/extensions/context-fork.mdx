...
Commands and skills normally run **inline** — their content is expanded and sent
...
Parsed but ignored. Tallow has no permission system — tool access is controlled
...
2. Passes `--model <model>` if a model is specified
