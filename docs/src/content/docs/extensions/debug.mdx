...
| Command | Description |
|---------|-------------|
| `/diag` | Show status (on/off, log path, size, last 5 entries) |
| `/diag-on` | Enable debug mode mid-session |
| `/diag-off` | Disable debug mode mid-session |
| `/diag-clear` | Truncate the log file |
| `/diag-tail [n]` | Show last N log entries (default: 20) |
