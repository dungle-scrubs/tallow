...
| `.claude/rules/*.md` | ✅ | Loaded by `context-files` (root and nested `.claude/rules/`). Supports both `path` and `paths` frontmatter. |
...
## Rules frontmatter compatibility (`path` and `paths`)

Rule files in `.claude/rules/` and `.tallow/rules/` share the same scope
frontmatter behavior.

Supported variants:

- `paths:` list of one or more glob patterns
- `path:` single glob pattern (Claude-style shorthand)

Activation semantics in both directories:

- no `path`/`paths` frontmatter → rule is unconditional
- with `path` or `paths` → rule activates only when the agent reads, edits,
  or writes a matching file path

This means you can move the same rule between `.claude/rules/` and
`.tallow/rules/` without rewriting scope metadata.
