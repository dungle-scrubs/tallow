...
- [x] Feature: `/clear` after deferred fork completion does not start a
...
- [x] Feature: new session starts idle with no hidden queued prompt.
- [x] Feature: old-session deferred triggers are cancelled or ignored
  after reset.
- [x] Feature: test output identifies which trigger source leaked.

### M2: Centralize reset transaction behavior
Source: `src/interactive-mode-patch.ts`,
`src/workspace-transition-interactive.ts`

- [x] Feature: `/clear` and workspace/session swaps share the same reset
  invariants.
- [x] Feature: one reset helper owns loader, status, pending message,
  and extension UI clearing.
- [x] Feature: resetting during compaction leaves no stale loader text.
- [x] Feature: resetting during forked or deferred work leaves no stale
  status text.
- [x] Feature: duplicate reset logic between interactive paths is
...
- [x] Feature: ordinary idle turn boundaries do not repaint the full
  transcript.
- [x] Feature: stale loader cleanup does not require unconditional
  `requestRender(true)` on every turn end.
- [x] Feature: rapid message end / agent end sequences keep transcript
  order stable.
- [x] Feature: reset-triggered rebuilds render once instead of a forced
  redraw cascade.
- [x] Feature: loader cleanup policy is testable independent of redraw
  policy.
...
- [x] Feature: reset lifecycle emits deterministic diagnostics in tests
  or debug mode.
- [x] Feature: deferred trigger registration is observable.
- [x] Feature: deferred trigger cancellation is observable.
- [x] Feature: late completions are recorded as dropped after reset,
  rather than silently starting work.
- [x] Feature: normal interactive mode stays quiet while debug evidence
  remains available.
...
- [x] Feature: dedicated `/clear` end-to-end transcript fixture passes.
- [x] Feature: `/clear` during compaction passes the end-to-end fixture.
- [x] Feature: `/clear` during deferred fork completion passes the
  end-to-end fixture.
- [x] Feature: targeted unit and integration suites pass.
- [x] Feature: `typecheck`, `typecheck:extensions`, `lint`, TUI build,
